...
import { formatBytes, formatDuration } from '@pxdl/core'
...
const Dashboard = () => {
  ...
  useInput(async (input, key) => {
    const activeTask =
      detailedTaskId !== null ? tasks.find((t) => t.id === detailedTaskId) : tasks[selectedIndex]
    ...
    if (detailedTaskId !== null) {
      ...
      if (key.return) {
        setDetailedTaskId(activeTask.id)
        return
      }
    }
  ...
  const renderTask = (task: DownloadTask, isSelected: boolean) => {
    ...
    const fixedTotal =
      ...
      (showStats ? sizeWidth + speedWidth + etaWidth : sizeWidth) +
      gap * 6
        ...
        <Box paddingX={1} marginBottom={0} key={task.id}>
          ...
          <Box flexGrow={1}>
            {isConfirmingFileDelete ? (
              <Text color='red' bold>
                Download complete. Also delete file from disk? (y/n)
              </Text>
            ) : (
              <Text color='red' bold>
                Confirm delete task {task.status !== 'completed' ? 'and partial file' : ''}? (y/n)
              </Text>
            )}
          </Box>
        </Box>
          ...
          <>
            ...
            <Box width={etaWidth} flexShrink={0}>
              <Text color={isSelected ? 'cyan' : 'yellow'}>
                {task.status === 'downloading' && task.eta !== undefined
    ...
    return (
      <Box flexDirection='column' padding={1} height='100%'>
        ...
        >
          {isConfirming ? (
            ...
            <>
              ...
              <Box marginTop={1}>
                <Text dimColor>URL: {task.url}</Text>
              </Box>
            </>
          )}
        </Box>
          ...
          {task.segments?.map((s, idx) => {
            ...
            return (
              <Box key={s.id} marginBottom={0} paddingX={1}>
                ...
                <Box width={4}>
                  <Text color={statusCfg.color}>{statusCfg.icon}</Text>
                </Box>
                <Box width={35}>
                  <ProgressBar progress={progress} width={25} color={statusCfg.color} />
                  <Text> {progress.toString().padStart(3)}%</Text>
                </Box>
                <Box width={15}>
                  <Text color='blue'>
                    {s.status === 'downloading' && s.speed ? `${formatBytes(s.speed)}/s` : '-'}
                  </Text>
                </Box>
              </Box>
            )
        ...
        <Text dimColor color='white'>
          {' '}
          (SPACE pause/resume, X delete, ESC return)
        </Text>
      </Box>
    )
  }
    ...
    <Box flexDirection='column' padding={1} height='100%'>
      <Box marginBottom={1} justifyContent='space-between'>
        <Box>
          ...
          <Text dimColor color='white'>
            {' '}
            (N new, ENTER detail, SPACE pause, X delete, Q exit)
          </Text>
        </Box>
        <Box>
        ...
        >
          ...
          {isConfirmingDuplicate ? (
            <Text color='yellow' bold>
              URL already in queue. Add anyway? (y/n)
            </Text>
          ) : (
            <Box>
              <Text>URL: </Text>
